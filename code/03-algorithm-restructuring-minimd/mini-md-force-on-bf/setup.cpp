...
int create_atoms(Atom &atom, int nx, int ny, int nz, double rho)
{
  ...
  int iflagall;
  MPI_Allreduce(&iflag, &iflagall, 1, MPI_INT, MPI_MAX, hosts_communicator);
...
void create_velocity(double t_request, Atom &atom, Thermo &thermo)
{
  ...
  double tmp;
  MPI_Allreduce(&vxtot, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  vxtot = tmp / atom.natoms;
  MPI_Allreduce(&vytot, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  vytot = tmp / atom.natoms;
  MPI_Allreduce(&vztot, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
