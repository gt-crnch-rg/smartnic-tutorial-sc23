            ...
            Thermo &thermo, Integrate &integrate, Timer &timer, int screen_yaml)
{
  ...
  int nlostall;
  MPI_Allreduce(&nlost, &nlostall, 1, MPI_INT, MPI_SUM, hosts_communicator);
  ...
  MPI_Allreduce(&time_total, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  ...
  MPI_Allreduce(&time_force, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_neigh, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_comm, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_other, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&tmp, &total, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
           ...
           int nhisto, int* histo)
{
  ...
  int ntotal;
  MPI_Allreduce(&n, &ntotal, 1, MPI_INT, MPI_SUM, hosts_communicator);
  double tmp;
  MPI_Allreduce(&ave, &tmp, 1, MPI_DOUBLE, MPI_SUM, hosts_communicator);
  ave = tmp / ntotal;
  MPI_Allreduce(&min, &tmp, 1, MPI_DOUBLE, MPI_MIN, hosts_communicator);
  min = tmp;
  MPI_Allreduce(&max, &tmp, 1, MPI_DOUBLE, MPI_MAX, hosts_communicator);
  max = tmp;
  ...
  MPI_Allreduce(histo, histotmp, nhisto, MPI_INT, MPI_SUM, hosts_communicator);
